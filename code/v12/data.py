...
def load_data_fold(config, valid_fold=1, sample=False):
    """Load Dataframe Using Fold

    Args:
        config: CFG
        valid_fold: fold index, choice [1, 2, 3, 4, 5]
    ...
    # dataset info path
    path = os.path.join(config.root_path, "dataset_info_5folds")
    ...
    for fold in train_fold:
        for i, algo in enumerate(algorithm):
            for j, qf in enumerate(qfs):
                names = pd.read_csv(os.path.join(path, f"train_dataset_info_qf{qf}_{fold}_5.csv"))['image_filename']
                names = names.values
                names = [os.path.join(config.root_path, algo, name) for name in names]
    ...
    for i, algo in enumerate(algorithm):
        for j, qf in enumerate(qfs):
            names = pd.read_csv(os.path.join(path, f"train_dataset_info_qf{qf}_{valid_fold}_5.csv"))['image_filename']
            names = names.values
            names = [os.path.join(config.root_path, algo, name) for name in names]
