...
# loss function
def loss_func_pre(pred, target):
    return nn.L1Loss()(pred, target)


class LearnerPre(object):
    ...
    def _train_one_epoch(self, train_loader, model, optimizer, scheduler):
        ...
        for X_stego, X_cover in train_iterator:
            X_stego = X_stego.to(self.config.device)
            X_cover = X_cover.to(self.config.device)

            batch_size = X_stego.size(0)
    ...
    def _valid_one_epoch(self, valid_loader, model):
        ...
        for i, (X_stego, X_cover) in enumerate(valid_loader):
            X_stego = X_stego.to(self.config.device)
            X_cover = X_cover.to(self.config.device)
...
class Learner(object):
    ...
    def predict(self, tst_data):
        ...
        for X_batch, _, _, _ in test_loader:
            ...
            with torch.no_grad():
                preds = model(X_batch, self.config.pretrain)
