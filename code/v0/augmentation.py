...
def fmix_loss(input, y1, index, lam, train=True, reformulate=False):
    ...
    if train and not reformulate:
        ...
        return LabelSmoothingCrossEntropy()(input, y1) * lam + LabelSmoothingCrossEntropy()(input, y2) * (1 - lam)
    else:
        return F.cross_entropy(input, y1)


def fmix_loss_be(input, y1, index, lam, train=True, reformulate=False):
    r"""Criterion for fmix
    Args:
        input: If train, mixed input. If not train, standard input
        y1: Targets for first image
    ...
    if train and not reformulate:
        y2 = y1[index]
        return F.binary_cross_entropy_with_logits(input, y1) * lam + F.binary_cross_entropy_with_logits(input, y2) * (
                    1 - lam)
    else:
        return F.binary_cross_entropy_with_logits(input, y1)
...
class FMix(FMixBase):
    ...
    def loss_be(self, y_pred, y, train=True):
        return fmix_loss_be(y_pred, y, self.index, self.lam, train, self.reformulate)
