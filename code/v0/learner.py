...
# loss function
def loss_func(pred, target, target_bin):
    weight = (target_bin == 0).type(torch.float) * 0 + 1

    pred, pred_bin = pred[:, :-1], pred[:, -1]
    loss_ce = (nn.CrossEntropyLoss(reduction='none')(pred, target) * weight).mean()
    loss_bin = (nn.BCEWithLogitsLoss(reduction='none')(pred_bin, target_bin) * weight).mean()
    return loss_ce, loss_bin
...
class Learner(object):
    def __init__(self, config):
        ...
        self.logger = None
        self.name = "model"
        self.fmix = FMix(size=(config.image_size, config.image_size))
    ...
    def predict(self, tst_data):
        ...
        for X_batch, typ, _, _ in test_loader:
            X_batch = X_batch.to(self.config.device)
            typ = typ.to(self.config.device)

            with torch.no_grad():
                preds = model(X_batch, typ)
    ...
    def _train_one_epoch(self, train_loader, model, optimizer, scheduler):
        ...
        for X_batch, typ, y_batch, y_bin in train_iterator:
            ...
            # fmix
            X_batch = self.fmix(X_batch)

            preds = model(X_batch, typ)

            # loss, loss_bin = loss_func(preds, y_batch.view(-1), y_bin.view(-1))
            loss = self.fmix.loss(preds[:, :-1], y_batch.view(-1))
            loss_bin = self.fmix.loss_be(preds[:, -1].view(-1), y_bin.view(-1))
            losses.update(loss.item(), batch_size)
            losses_bin.update(loss_bin.item(), batch_size)

            optimizer.zero_grad()
            (loss + loss_bin).backward()
            optimizer.step()
            scheduler.step()

            train_iterator.set_description(
                f"train ce:{losses.avg:.4f}, {losses_bin.avg:.4f} lr:{scheduler.get_lr()[0]:.4f}")

        return losses.avg, losses_bin.avg
    ...
    def _valid_one_epoch(self, valid_loader, model):
        ...
        for i, (X_batch, typ, y_batch, y_bin) in enumerate(valid_loader):
            ...
            valid_loader.set_description(f"valid ce:{losses.avg:.4f}, {losses_bin.avg:.4f}")

        true_final = torch.cat(true_final, dim=0).numpy()
        true_final = (true_final != 0) * 1

        pred_final = torch.cat(pred_final, dim=0)
        pred_final_sum = nn.Softmax()(pred_final[:, :-1])
        pred_final_sum = pred_final_sum[:, 1:].sum(-1).numpy()

        vl_score = self._cal_metrics(pred_final_sum, true_final)

        return losses.avg, losses_bin.avg, vl_score

    def _create_logger(self):
        log_cols = ['tr_loss', 'tr_loss_bin', 'val_loss', 'val_loss_bin', 'val_metric']
