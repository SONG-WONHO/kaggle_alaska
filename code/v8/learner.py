...
class Learner(object):
    ...
    def _train_one_epoch(self, train_loader, model, optimizer, scheduler):
        ...
        for X_batch, typ, y_batch, y_bin in train_iterator:
            X_batch = X_batch.to(self.config.device)
            y_batch = y_batch.to(self.config.device).repeat(17)
    ...
    def _valid_one_epoch(self, valid_loader, model):
        ...
        pred_final_sum = nn.Softmax(dim=-1)(pred_final)
        pred_final_sum = pred_final_sum[:, :, 1:].sum(dim=-1).numpy()

        vl_score = []
        for i in range(17):
            vl_score.append(self._cal_metrics(pred_final_sum[:, i], true_final[:, i]))
